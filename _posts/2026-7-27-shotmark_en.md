---
layout:     post
title:      "ShotMark User Guide: Add Camera Info Watermarks on Mac"
subtitle:   "macOS, RAW, photo watermark, camera settings, ShotMark"
date:       2026-7-27
author:     "Karim"
...
ShotMark is a native macOS photo library and camera-info watermark app. It reads details such as camera, lens, focal length, aperture, shutter speed, and ISO from a photo, then presents them in a clean layout around the image.

ShotMark supports JPEG, PNG, HEIC, and RAW formats that the installed macOS Core Image RAW decoder can open. You can browse a group of photos, apply a template to one or many images, and export finished files as JPEG, PNG, or HEIC.
...
- ShotMark includes English and Simplified Chinese and follows the app language selected in macOS.

# Add Photos

Open ShotMark and click `Add Photos`, or drag photos directly into the library window.

ShotMark keeps access information for each source file without copying, moving, or modifying the original photo. If the same source file is imported again, you can skip the duplicate, import it anyway, or cancel the import.
...
ShotMark includes two watermark templates:
...
These choices are stored only in the ShotMark library. They are not written back to the photo's EXIF metadata and do not alter the source file.

# Select and Process Multiple Photos

ShotMark follows standard macOS selection behavior:
...
Choosing `Remove` deletes only the ShotMark library records and thumbnail cache. It does not delete, move, or modify the source photos, and the operation can be undone with `Command-Z`.
...
The default output name is `original-name-shotmark.extension`. If that name already exists, ShotMark adds `-2`, `-3`, and so on instead of overwriting a file.

You can cancel a batch export. Files already completed are kept, and items that have not started are stopped. A failure on one photo does not stop the remaining photos; ShotMark shows a summary when the batch finishes.

# Relocate an Unavailable File

Because ShotMark works with the original photo rather than an internal copy, a file may become unavailable after it is moved or renamed, or while an external drive is disconnected.
...
- ShotMark does not upload photos and has no cloud-sync feature.
- Original photos are not copied into the ShotMark library.
- Camera-info edits remain in ShotMark's local index.
- Removing an item from the library does not delete the source photo.
- Export never overwrites an imported source photo.

# Q&A

1. **Why can’t ShotMark preview a RAW file?**
...
2. **Does editing camera information change the photo's EXIF metadata?**
   No. Custom and hidden settings affect only ShotMark previews and exports.

3. **Does Remove from Library delete the file from my drive?**
   No. It removes only ShotMark's library record and thumbnail cache.

4. **Can batch export overwrite an existing image?**
   No. ShotMark automatically adds a numeric suffix when a filename is already in use.
...
6. **Why does a photo show “File unavailable” after I move it?**
   ShotMark uses the source file and does not keep a separate photo copy. Choose `Relocate...` to reconnect the moved file.
